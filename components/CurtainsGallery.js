...
export default function CurtainsGallery() {
  ...
  return (
    <section>
      <div id="canvas" className="fixed top-0 left-0 w-full h-full z-20 pointer-events-none"></div>
      <div
        id="planes"
        className="px-6 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 h-full relative z-10"
      >
        {images.map((item, index) => (
          <div
            key={index}
            className="plane aspect-[2/3]  relative" // Changed to h-[70vh]
          >
