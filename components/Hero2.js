...
export default function Hero2() {
  ...
  useEffect(() => {
    ...
    const updateHeight = () => {
      ...
      // include all descendants because some are absolutely positioned
      const all = el.querySelectorAll("*");
      all.forEach((node) => {
        if (!(node instanceof Element)) return;
        const r = node.getBoundingClientRect();
        // ignore nodes with zero area
        if (r.width === 0 && r.height === 0) return;
        const bottom = r.bottom - parentRect.top;
        if (bottom > maxBottom) maxBottom = bottom;
      });

      // fallback to natural content height if nothing found
      if (!maxBottom) maxBottom = el.scrollHeight;

      // apply measured height (round up)
      el.style.height = `${Math.ceil(maxBottom)}px`;
    };

    // Recalculate on resize
    window.addEventListener("resize", updateHeight);

    // Recalculate when images load (images may change layout)
    const imgs = el.querySelectorAll("img");
    imgs.forEach((img) => img.addEventListener("load", updateHeight));

    // Watch DOM changes (class changes, new nodes, etc)
    ...
    // Initial calc (also allow a microtask so SSR->hydration image sizes settle)
    updateHeight();
  ...
  return (
    <section>
      <div
        ref={heroRef}
        className="relative flex flex-col lg:flex-row w-full h-auto"
      >
        <div className="absolute flex flex-col gap-1 md:flex-col justify-between text-[#343434] right-12 md:right-16 top-4 md:top-7">
          <a href="mailto:Olaoluwaakinwale89@gmail.com" className="group">
            <h5 className="text-xs text-right md:text-sm text-[#343434]">
              <span
                className=" underline-offset-2 underline lg:no-underline
        relative
      ...
      "
          ...
          <div className="flex flex-row justify-between md:justify-end pt-4 md:pt-3">
            ...
            <div className="text-left flex flex-col gap-3 pt-5 text-[#343434]">
              {[
                ...
                <div
                  key={item.id}
                  className="flex self-end  underline-offset-2 underline lg:no-underline flex-row gap-1 cursor-pointer group"
                  onClick={() => scrollToSection(item.id)}
                >
                  <h5 className="text-base md:text-xl relative overflow-hidden group">
                    <span className="relative after:absolute after:left-0 after:bottom-0 after:h-[2px] after:w-full after:bg-[#343434] after:origin-left after:scale-x-0 group-hover:after:scale-x-100 after:transition-transform after:duration-300">
                      {item.label}
                    </span>
                  </h5>
                  <svg
        ...
        <div className="hidden lg:block">
          <div className="flex absolute right-16 top-32 -z-10 flex-row-reverse gap-4">
